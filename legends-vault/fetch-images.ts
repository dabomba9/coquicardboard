/**
 * Download a player's TCDB front thumbnails (legends-vault/data/<player>/image-urls.json)
 * through the user's CDP-attached Chrome (TCDB is Cloudflare-walled) and upload each
 * to the shared `vault-images` Storage bucket as {cid}-front.jpg. Then run
 * admin/reconcile-vault-images.ts <catalog> tcdb to set cards.image_url.
 *
 *   # Chrome (solve Cloudflare, leave open):
 *   "/Applications/Google Chrome.app/Contents/MacOS/Google Chrome" \
 *     --remote-debugging-port=9222 --user-data-dir="$HOME/.cf-chrome" "https://www.tcdb.com/"
 *
 *   npx tsx legends-vault/fetch-images.ts --player clemente --limit 20  # test first
 *   npx tsx legends-vault/fetch-images.ts --player clemente             # full (resumable)
 */
...
// Front cids already in the bucket (resumable: skip them). The bucket is shared
// across all vaults, so we only mark cids from THIS player's url list.
...
async function main() {
  const all: { cid: number; url: string }[] = JSON.parse(readFileSync(join(HERE, "data", player.key, "image-urls.json"), "utf8"));
  let browser;
  ...
  const doneBefore = all.filter((x) => done.has(x.cid)).length;
  const pending = force ? all : all.filter((x) => !done.has(x.cid));
  const todo = pending.slice(0, Number.isFinite(limit) ? limit : undefined);
  console.log(`${player.name} thumbnails: ${doneBefore}/${all.length} already in bucket.${force ? " --force: re-fetching all." : ""} Fetching ${todo.length}…`);
  ...
  if (blocked) console.log("⚠ 25 failures in a row — the Cloudflare session likely expired. Re-solve at tcdb.com in the Chrome window, then re-run (resumable).");
