...
export function NavBar({ authed, admin }: { authed: boolean; admin: boolean }) {
  ...
  return (
    <header className="font-modern fixed inset-x-0 top-0 z-30 px-3 pt-3 sm:px-4">
      <nav className="mx-auto flex h-16 max-w-5xl items-center gap-3 rounded-full border border-border/45 bg-background/65 px-3 shadow-[0_8px_30px_-12px_rgba(0,0,0,0.30)] backdrop-blur-xl sm:px-5">
        ...
        <div className="ml-auto flex items-center gap-1.5 sm:gap-2">
          ...
          <div className="hidden items-center gap-2 md:flex">
            {authed ? (
              <>
                <Link href="/settings" className="rounded-full px-3 py-1.5 text-sm font-medium text-muted transition-colors hover:bg-foreground/5 hover:text-foreground">Settings</Link>
                <form action={signOut}>
                  <button className="rounded-full border border-border/60 px-4 py-1.5 text-sm font-medium text-foreground transition-colors hover:bg-foreground/5">Sign out</button>
                </form>
              </>
            ) : (
              <Link href="/login" className="rounded-full bg-accent px-4 py-1.5 text-sm font-semibold text-black transition hover:brightness-105">Sign in</Link>
            )}
          </div>
          ...
          <button
            ...
            aria-expanded={open}
            onClick={() => setOpen((v) => !v)}
            className="inline-flex h-9 w-9 items-center justify-center rounded-full text-muted transition-colors hover:bg-foreground/5 hover:text-foreground md:hidden"
          >
      ...
      {open && (
        <div className="mx-auto mt-2 max-w-5xl rounded-2xl border border-border/40 bg-background/90 px-4 py-3 shadow-[0_10px_34px_-12px_rgba(0,0,0,0.40)] backdrop-blur-xl md:hidden">
