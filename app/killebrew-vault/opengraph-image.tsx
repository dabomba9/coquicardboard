...
export const alt = "The Killebrew Vault · Coqui Cardboard";
...
export default async function Image() {
  ...
  let subtitle = "Harmon Killebrew's playing-era run";
  try {
    const { total, withImage } = await getCatalogCounts("killebrew-vault");
    if (total) subtitle = `${total.toLocaleString()} cards · ${withImage.toLocaleString()} with images`;
  } catch {}
  return catalogOgImage("The Killebrew Vault", subtitle);
}
