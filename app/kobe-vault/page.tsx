...
export const metadata: Metadata = {
  title: "Kobe Vault — every Kobe Bryant card",
  description: "Browse the complete Kobe Bryant card catalog — search, filter, follow market value, and track what you own.",
  alternates: { canonical: "/kobe-vault" },
  openGraph: {
    title: "Kobe Vault — every Kobe Bryant card · Coqui Cardboard",
    description: "Browse the complete Kobe Bryant card catalog — search, filter, follow market value, and track what you own.",
    url: "/kobe-vault",
    type: "website",
  },
...
export default async function KobeVaultPage({
  searchParams,
}: {
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-10">
      <JsonLd data={collectionJsonLd({ name: "Kobe Vault", description: metadata.description as string, path: "/kobe-vault", items: tiles.map((t) => ({ name: t.name ?? "", slug: t.slug })) })} />
      <h1 className="font-display text-lg uppercase tracking-tight">Kobe Vault</h1>
      <p className="mt-1 text-sm text-muted">
        <span className="font-data text-base text-foreground">{catalog.length.toLocaleString()}</span> Kobe Bryant
      ...
      <VaultExplorer
        ...
        player={playerForCatalog("kobe-vault")}
      />
    </div>
  );
}
