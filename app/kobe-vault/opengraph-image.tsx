...
export const alt = "The Kobe Vault · Coqui Cardboard";
...
export default async function Image() {
  ...
  let subtitle = "Every Kobe Bryant issue";
  try {
    const { total, withImage } = await getCatalogCounts("kobe-vault");
    if (total) subtitle = `${total.toLocaleString()} cards · ${withImage.toLocaleString()} with images`;
  } catch {}
  return catalogOgImage("The Kobe Vault", subtitle);
}
