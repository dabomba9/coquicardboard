...
export const alt = "Mamba Origins · Coqui Cardboard";
...
export default async function Image() {
  ...
  let subtitle = "Kobe Bryant's 1996-97 rookie class";
  try {
    const { total, withImage } = await getCatalogCounts("kobe-hierarchy");
    if (total) subtitle = `${total.toLocaleString()} cards · ${withImage.toLocaleString()} with images`;
  } catch {}
  return catalogOgImage("Mamba Origins", subtitle);
}
